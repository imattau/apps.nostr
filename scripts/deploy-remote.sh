...
APP_NAME="apps.nostr"
SERVICE_NAME="apps-nostr"
INSTALL_DIR="/var/www/apps.nostr"
...
SKIP_BUILD=false
SSH_TARGET=""
SSH_CONTROL_PATH="${TMPDIR:-/tmp}/apps-nostr-ssh-%C"
...
render_header() {
	...
	printf '\n'
	hr
	panel_line "$(status_info "${APP_NAME} deploy installer")"
	hr
...
Options:
  ...
  --install-dir <path>     Remote install path (default: /var/www/apps.nostr)
...
Environment overrides:
  APPS_NOSTR_SSH_TARGET, APPS_NOSTR_PORT, APPS_NOSTR_INSTALL_DIR,
  APPS_NOSTR_SERVICE_USER, APPS_NOSTR_SERVICE_GROUP, APPS_NOSTR_PROXY,
  APPS_NOSTR_DOMAIN, APPS_NOSTR_CADDY_EMAIL, APPS_NOSTR_SSH_PORT,
  APPS_NOSTR_DRY_RUN, APPS_NOSTR_SKIP_BUILD
EOF
}
...
managed_marker="# managed by apps.nostr deploy"
...
printf '\n'
hr
panel_line "apps.nostr remote installer"
hr
panel_line "install  : ${INSTALL_DIR}"
panel_line "service  : ${SERVICE_NAME}"
panel_line "port     : ${PORT}"
...
detect_caddy_snippet_dir() {
	...
	local dir
	if [[ -n "${APPS_NOSTR_CADDY_SNIPPET_DIR:-}" ]]; then
		printf '%s' "$APPS_NOSTR_CADDY_SNIPPET_DIR"
		return 0
	fi
	if [[ -f "$main_file" ]]; then
...
${import_line}"
		...
		if grep -qE '^[[:space:]]*import[[:space:]].*(/etc/caddy/)?(conf\.d|Caddyfile\.d)/\*\.caddy' "$main_file"; then
			...
			log "existing Caddyfile is managed by apps.nostr; adding snippet import"
...
detect_nginx_style() {
	...
	if [[ -n "${APPS_NOSTR_NGINX_STYLE:-}" ]]; then
		printf '%s' "$APPS_NOSTR_NGINX_STYLE"
		return 0
	fi
...
EOF
	...
	ssh -tt -p "$SSH_PORT" \
		...
		-o "ControlPersist=10m" \
		-o "ControlPath=${SSH_CONTROL_PATH}" \
		"$SSH_TARGET" "bash /tmp/${SERVICE_NAME}-deploy.sh $(printf '%q %q %q %q %q %q %q %q %q' \
			"$INSTALL_DIR" "$REMOTE_STAGE_DIR" "$SERVICE_NAME" "$SERVICE_USER" "$SERVICE_GROUP" "$PORT" "$PROXY_MODE" "$DOMAIN" "$CADDY_EMAIL")"
	step_done
}
...
parse_env() {
	SSH_TARGET="${APPS_NOSTR_SSH_TARGET:-${SSH_TARGET:-}}"
	PORT="${APPS_NOSTR_PORT:-${PORT:-3000}}"
	INSTALL_DIR="${APPS_NOSTR_INSTALL_DIR:-${INSTALL_DIR:-/var/www/apps.nostr}}"
	SERVICE_USER="${APPS_NOSTR_SERVICE_USER:-${SERVICE_USER:-www-data}}"
	SERVICE_GROUP="${APPS_NOSTR_SERVICE_GROUP:-${SERVICE_GROUP:-www-data}}"
	PROXY_MODE="${APPS_NOSTR_PROXY:-${PROXY_MODE:-auto}}"
	DOMAIN="${APPS_NOSTR_DOMAIN:-${DOMAIN:-}}"
	CADDY_EMAIL="${APPS_NOSTR_CADDY_EMAIL:-${CADDY_EMAIL:-}}"
	SSH_PORT="${APPS_NOSTR_SSH_PORT:-${SSH_PORT:-22}}"
	if [[ -n "${APPS_NOSTR_DRY_RUN:-}" ]] && is_true "${APPS_NOSTR_DRY_RUN}"; then
		DRY_RUN=true
	fi
	if [[ -n "${APPS_NOSTR_SKIP_BUILD:-}" ]] && is_true "${APPS_NOSTR_SKIP_BUILD}"; then
		SKIP_BUILD=true
	fi
}
